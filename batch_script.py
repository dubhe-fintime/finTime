        ...
        with app.app_context():
            ...
            for (task_name, response) in zip(task_futures.keys(), responses):
                ...
                if isinstance(response, Exception):
                    ...
                    # 배치 로그 DB 저장
                    set_batch_log(BATCH_ID , BATCH_NM, res['fin_id'], task_name, now, task_time, "FAIL", response)
                else:
                    ...
                    # 배치 로그 DB 저장
                    set_batch_log(BATCH_ID , BATCH_NM, res['fin_id'], task_name, now, task_time, status, json.dumps(res['result'], ensure_ascii=False))
